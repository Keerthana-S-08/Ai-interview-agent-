...
function App() {
  ...
  async function startInterview() {
    ...
    try {
      ...
      setQuestionCount(data.progress?.questionsAsked || 1);
      setAnsweredCount(data.progress?.questionsAnswered || 0);
  ...
  async function submitAnswer() {
    ...
    try {
      ...
      if (data.done) {
        ...
        setQuestionCount(data.progress?.questionsAsked || TOTAL_QUESTIONS);
        setAnsweredCount(data.progress?.questionsAnswered || TOTAL_QUESTIONS);
        setCoveredDays(data.progress?.coveredDays || []);
        
        
      } else {
        setMessages(prev => [...prev, { role: "ai", text: data.reply }]);
        setQuestionCount(data.progress?.questionsAsked || 0);
        setAnsweredCount(data.progress?.questionsAnswered || 0);
        setCoveredDays(data.progress?.coveredDays || []);
    ...
    <div className="app-shell">
      ...
      <main className="interview-layout">
        ...
        <aside className="metrics">
          <div className="metric-card"><div className="metric-icon"><Target size={18}/></div><span>Questions asked</span><strong>{questionCount}/{TOTAL_QUESTIONS}</strong><small>{answeredCount} answered</small></div>
          <div className="metric-card"><div className="metric-icon"><GraduationCap size={18}/></div><span>Curriculum days</span><strong>{coveredDays.length} covered</strong><small>{coveredDays.length >= 4 ? "4+ requirement met" : "Building coverage..."}</small></div>
