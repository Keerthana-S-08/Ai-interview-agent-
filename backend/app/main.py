...
def evaluate_answer(session: Dict[str, Any], answer: str) -> Dict[str, Any]:
    ...
    if not answer.strip():
        quality = "weak"
        note = "No meaningful answer was provided."
    elif answer.strip().lower() in {
        "i don't know",
        "i dont know",
        "don't know",
        "dont know",
        ...
        quality = "weak"
        note = "The candidate explicitly indicated that they did not know the answer."
    elif words >= 45:
        quality = "strong"
        note = "The response provided sufficient detail for a meaningful technical evaluation."
    elif words >= 15:
        quality = "mixed"
        note = "The response showed partial understanding but could use more technical depth."
    else:
        quality = "weak"
        note = "The response was too brief to demonstrate sufficient technical understanding."

    return {"quality": quality, "note": note}
    prompt = f"""
...
def interview(req: InterviewRequest):
    ...
    "feedback": feedback,
    "progress": {
        "questionsAsked": len(session["asked_questions"]),
        "questionsAnswered": len(session["evaluations"]),
        "questionsTotal": QUESTION_LIMIT,
        "daysCovered": len(session["covered_days"]),
